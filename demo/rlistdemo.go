...
func genRandList(mo recursivelist.Model[rListItem], deleg list.ItemDelegate, maxDepth *int, curDepth int) ([]recursivelist.ListItem[rListItem], []rListItem) {
	...
	for i := 0; i < rand.Intn(21); i++ {
		...
		*cv.ParentModel = mo
		if curDepth < *maxDepth {
			curDepth++
			sts, (cv.Value).children = genRandList(mo, deleg, maxDepth, curDepth)
			// fmt.Printf("%+v\n", sts)
...
func initRlistModel() recursivelist.Model[rListItem] {
	...
	rlisto, _ := genRandList(m, rListDelegate{}, &bla, 0)
	m.SetItems(rlisto)
